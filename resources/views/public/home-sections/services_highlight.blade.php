    ...
    <div class="grid {{ $gridClass }}">
        @foreach($items as $item)
            @php
                ...
                $name        = $service ? addslashes($service->name()->get(app()->getLocale())) : '';
                $description = $service ? addslashes(strip_tags(Str::limit($service->description()->get(app()->getLocale()), 200))) : '';
                $coverUrl    = $service ? ($service->coverUrl() ?? '') : '';
                $url         = $service ? url('/serveis/' . $service->slug()) : '';
            @endphp
            @if($service)
                <div
